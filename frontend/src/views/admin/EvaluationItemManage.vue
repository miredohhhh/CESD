  ...
  <el-card>
    <template #header>
      <div class="page-header">
        <span>综测项目管理</span>
        <el-button v-if="canCreate" type="primary" @click="openCreateDialog">新增项目</el-button>
      </div>
    </template>

    <el-form :model="filters" inline>
      <el-form-item label="关键字">
        <el-input v-model="filters.keyword" clearable placeholder="项目名称 / 编码 / 计分方式" />
      </el-form-item>
    ...
    <el-table v-loading="loading" :data="records" border>
      ...
      <el-table-column prop="itemCode" label="项目编码" min-width="130" />
      <el-table-column label="所属分类" min-width="150">
        <template #default="{ row }">{{ getCategoryName(row.categoryId) }}</template>
      </el-table-column>
      <el-table-column prop="scoreType" label="计分方式" width="110" />
      <el-table-column prop="score" label="默认分" width="90" />
      <el-table-column prop="maxScore" label="最高分" width="90" />
      <el-table-column label="附件" width="90">
        <template #default="{ row }">{{ row.needAttachment === 1 ? '需要' : '不需要' }}</template>
      </el-table-column>
    ...
    <el-dialog v-model="dialogVisible" :title="editingId ? '编辑项目' : '新增项目'" width="620px">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="需要附件" prop="needAttachment">
          <el-select v-model="form.needAttachment" style="width: 100%">
