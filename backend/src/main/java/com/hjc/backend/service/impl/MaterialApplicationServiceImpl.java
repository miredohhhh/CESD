...
public class MaterialApplicationServiceImpl extends ServiceImpl<MaterialApplicationMapper, MaterialApplication> implements MaterialApplicationService {
    ...
    public MaterialApplicationVO create(CreateMaterialApplicationRequest request) {
        Long studentId = requireCurrentStudentIdForCreate();
    ...
    public MaterialApplicationVO approveMaterialApplication(Long id, ApproveMaterialApplicationRequest request) {
        ...
        checkReviewerExists(reviewerId);
    ...
    public PageResult<MyMaterialApplicationVO> pageMyApplications(MyMaterialApplicationPageRequest request) {
        ...
        LambdaQueryWrapper<MaterialApplication> wrapper = new LambdaQueryWrapper<>();
        wrapper.eq(MaterialApplication::getStudentId, studentId)
    ...
    @Override
    public MyApplicationStatisticsVO getMyApplicationStatistics() {
        Long effectiveStudentId = CurrentUserUtils.requireStudentId();
        checkStudentExists(effectiveStudentId);
        List<MaterialApplication> applications = lambdaQuery()
                .eq(MaterialApplication::getStudentId, effectiveStudentId)
                .list();
        MyApplicationStatisticsVO vo = new MyApplicationStatisticsVO();
        vo.setStudentId(effectiveStudentId);
    ...
    public PageResult<PendingMaterialApplicationVO> pagePendingApplications(PendingMaterialApplicationPageRequest request) {
        CurrentUserUtils.requireAuditorOrAdmin();
        ...
        List<Long> studentIds = intersectStudentIds(listFilteredStudentIds(request), listPermittedAuditStudentIds());
    ...
    private Long requireCurrentStudentIdForCreate() {
        CurrentUserUtils.requireUserId();
        Long studentId = CurrentUserUtils.getStudentId();
        if (studentId == null) {
            throw new BusinessException("当前账号未绑定学生信息，无法创建申报");
        }
        return studentId;
    }
    ...
    private List<Long> listPermittedAuditStudentIds() {
        if (CurrentUserUtils.isAdmin()) {
            return null;
        }
        if (!CurrentUserUtils.isAuditor()) {
            throw new BusinessException(ResultCode.FORBIDDEN, "Current user has no audit permission");
        }
        List<ReviewerScope> scopes = reviewerScopeService.lambdaQuery()
                .eq(ReviewerScope::getReviewerId, CurrentUserUtils.requireUserId())
                .list();
        if (scopes.isEmpty()) {
            return List.of();
        }
        Set<Long> permittedStudentIds = new HashSet<>();
        for (ReviewerScope scope : scopes) {
            boolean hasScopeCondition = scope.getMajorId() != null
                    || scope.getClassId() != null
                    || StringUtils.hasText(scope.getGrade());
            if (!hasScopeCondition) {
                // TODO Define whether an empty reviewer_scope row should mean all data. Current strategy is conservative.
                continue;
            }
            LambdaQueryWrapper<Student> wrapper = new LambdaQueryWrapper<>();
            wrapper.eq(scope.getMajorId() != null, Student::getMajorId, scope.getMajorId())
                    .eq(scope.getClassId() != null, Student::getClassId, scope.getClassId())
                    .eq(StringUtils.hasText(scope.getGrade()), Student::getGrade, scope.getGrade());
            permittedStudentIds.addAll(studentService.list(wrapper).stream().map(Student::getId).toList());
        }
        return permittedStudentIds.stream().toList();
    }

    private List<Long> intersectStudentIds(List<Long> filterStudentIds, List<Long> scopeStudentIds) {
        if (filterStudentIds == null) {
            return scopeStudentIds;
        }
        if (scopeStudentIds == null) {
            return filterStudentIds;
        }
        Set<Long> scopeSet = new HashSet<>(scopeStudentIds);
        return filterStudentIds.stream().filter(scopeSet::contains).toList();
    }
